...
> Generated fonts land in `fonts/output/` (gitignored); committed assets land in
> `docs/images/`, `docs/specimen/`, and the golden manifest in `fonts/golden/`.
...
footer never go stale.
Source: `docs/specimen/planetaire-mono-specimen.typ`.

## 5. HTML specimen + static site
...
| `.git` | ~33 MB |
| `fonts/source/` (Hack ~24 MB + B612 ~5 MB) | ~29 MB |
| `fonts/golden/manifest.json.gz` | ~2 MB |
| `docs/images/` + specimen PDF | ~0.9 MB |
...
dependency (and require `lfs: true` in CI) for no real benefit at this size.
The one genuinely oversized artifact — the former 13 MB golden manifest — was the thing
worth fixing, and it is now gzipped to ~2 MB.
