// Shared specimen content: palettes, helpers, and the reusable blocks used by
// BOTH the PDF specimen (planetaire-mono-specimen.typ) and the README image cards
// (card.typ). Defining them once keeps the home-page images in sync with the PDF.
...
// Syntax-highlighted Python sample (manual Kerm-colored tokens).
#let orbit-code() = code-block((
...
// Mock terminal session (eza --icons, python, git log) in the Kerm dark theme.
#let terminal-mockup() = block(
  fill: kerm.bg,
  inset: (x: 1.2em, y: 1em),
  radius: 4pt,
  width: 100%,
...
#let turing-passage(size: 10.5pt) = text(size: size)[
...
// Weight ladder: every variant on a sample line + a digits/symbols line.
#let weight-ladder() = {
...
// Legibility: confusable-character disambiguation + the dotted-zero variants.
#let legibility-pairs() = {
  ...
  grid(
    ...
    [
      ...
      #v(0.1cm)
      #text(size: 36pt)[0] #h(0.5cm) #text(size: 18pt)[0 O o]
      #v(0.1cm)
